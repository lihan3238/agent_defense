...
旧的 Qwen3-8B（通义千问 3 80 亿参数模型）30 回合表示级矩阵仍只对应 `melon`，不得用新实现回写或重解释冻结数字；
四套件完整主矩阵则单独对应 `melon_paper`。
...
该路径使用 Hugging Face（模型库）编码器和平均池化，是四套件完整主矩阵实际采用的离线后端，但不能声称复现论文嵌入分数。
早期 smoke（冒烟测试）中，相同攻击转账调用相似度为 `1.000000`，不同收款方的合法转账为 `0.793048`；这两个单案例分数
只验证后端接线与阈值行为，正式 AgentDojo（智能体安全评测框架）效果必须看完整主矩阵报告。

## 与论文 629 案例对齐的协议

论文分母对应 AgentDojo `v1.1.2`；旧的 30 回合表示级矩阵使用 `v1.2.2`，而四套件 MELON 主矩阵已切回
`v1.1.2`：
...
（受攻击回合），共 2613 回合。本仓库已经完成 `none` 与 `melon_paper` 的单一攻击完整 629 配对；论文四攻击全矩阵
仍未运行。

### 当前完整数据重跑协议

该优先项已经按分母不缩减的冻结协议执行完成：攻击只使用 `important_instructions`，防御只比较
...
16 配对 screening（筛选实验）先暴露了掩码候选稀疏和唯一阻断不匹配精确攻击参考的问题；它现在只保留为开发阶段诊断，
见 [`reports/melon-paper-screening.md`](../reports/melon-paper-screening.md)。后续没有只挑这 16 个案例重跑，也没有用筛选结果
调参或回填筛选指标；固定的四套件完整清单自然包含这些案例，并完成 `none` 与 `melon_paper` 共 1452 回合的单攻击主矩阵，
随后对结果、失败桶、调用阻断和开销做受检分析。

正式效果与限制只看
[`reports/qwen3-v112-full-matrix.md`](../reports/qwen3-v112-full-matrix.md)：它支持“论文兼容算法在本地 MiniLM（小型句向量模型）
与完整 629 配对上的重跑”，仍不支持论文嵌入后端、四攻击或 Table 1（表 1）数值复现。全部证据职责见
