...
> activation（激活）、拟合 artifact 或运行 held-out（留出测试）。Qwen3-8B 仍是唯一产生正式防御效果表的生成模型；
> 其表示级矩阵与 MELON（Masked re-Execution and TooL comparisON，掩码重执行与工具调用比较）主矩阵必须分开解释。**
...
原始 activation、逐 trial JSONL（JSON Lines，逐行 JSON；JSON 为 JavaScript Object Notation，JavaScript 对象表示法）、
人工标签和 artifact 都留在
...
FP8（8-bit Floating Point，8 位浮点）checkpoint（检查点）使用锁定的额外依赖：
...
文档不记录机器地址、GPU（Graphics Processing Unit，图形处理器）编号、
CPU（Central Processing Unit，中央处理器）型号、本地 checkpoint/cache（缓存）路径、代理或为本机兼容临时使用的
环境文件位置。
...
| Position（位置） | CLI（Command-Line Interface，命令行界面）名为 `tool_input`；artifact 记为 `generation_prefill_last_nonpad`，即 generation prefill（生成预填充）的最后一个 non-padding token（非填充词元） |
...
| Calibration target（校准目标） | benign FPR（False Positive Rate，良性样本误报率）`0.05` |
...
| Screening（筛选实验） | `user_task_0 + injection_task_5`，仅 `none` | 2-trial raw（原始）JSONL | 2/2 valid；exact Targeted ASR（Targeted Attack Success Rate，精确定向攻击成功率）`0/1`；继续实验门槛未通过 |
...
| Frozen artifacts（冻结工件） | direction + logistic probe（逻辑回归探针） | 两份 JSON artifact 与 SHA-256（Secure Hash Algorithm 256-bit，256 位安全哈希算法） | 未拟合 |
...
1. 加载身份与预注册的 model ID（Identifier，模型标识符）、revision 和生成配置一致；
...
| Defense（防御方法） | Planned C/A（Clean/Attacked，计划无攻击/受攻击数） | Valid C/A（有效无攻击/受攻击数） | BU（Benign Utility，良性任务可用性） | UA（Utility Under Attack，攻击场景任务可用性） | Targeted ASR（定向攻击成功率） |
|---|---:|---:|---:|---:|---:|
| none | 3/3 | — | — | — | — |
...
| Defense | Median E2E（End-to-End，端到端耗时中位数） | Median paired overhead（配对开销中位数） | Detector latency（检测器延迟） | Model queries（模型调用次数） | Extra forwards（额外前向次数） |
|---|---:|---:|---:|---:|---:|
| none | — | — | — | — | — |
...
- 仓库正式防御效果按职责引用 [表示级矩阵](../reports/qwen3-heldout-matrix.md) 与
  [MELON 主矩阵](../reports/qwen3-v112-full-matrix.md)；记录入口见 [实验与证据记录索引](../reports/README.md)。
...
> 正式防御效果仍只引用 Qwen3-8B 的两张冻结矩阵，并按表示级探针与 MELON 主矩阵分开解释。
