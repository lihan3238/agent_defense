...
> 模型提出的危险工具调用，怎样携带隐藏状态到达 gate，并在
> `FunctionsRuntime.run_function` 之前被允许或阻断？

先建立最重要的心智模型：LLM 负责**提出候选动作**，detector 负责**产生风险信号**，
`RuntimeGate` 负责**形成决策**，`GuardedToolsExecutor` 才负责**执行该决策**。
...
第一次 LLM 调用先产生候选动作；之后循环执行“executor 处理调用 → LLM 读取工具结果”。

## 2. 两条数据通道

`HuggingFaceToolCallingLLM.query` 返回两类相关数据，它们不是同一个对象：

| 数据 | 传递位置 | 含义 |
|---|---|---|
| candidate tool calls | assistant message 的 `tool_calls` | 模型想做什么 |
| activation + metadata | `extra_args/state["agent_defense.activation"]` | 模型提出动作前的内部状态 |

`GuardedToolsExecutor.query` 把两者合并成 `DetectionContext`。因此 detector 能读取隐藏状态，
policy 又能看到实际工具名和参数风险。
...
1. [`types.py`](../src/agent_defense/types.py)
   ...
   - `PolicyDecision`：allow/block、原因和风险级别；
   - `DecisionTrace`：调用是否越过 runtime boundary。

2. [`experiments.py`](../src/agent_defense/experiments.py)

   - `build_hf_experiment_pipeline`：把 defense 名映射到 detector、MELON provider 或 AgentDojo built-in；
   - `run_hf_agentdojo_case`：运行一个 clean/attacked episode，并收集 utility、ASR、trace 和开销。

3. [`hf_llm.py`](../src/agent_defense/hf_llm.py)

   - `HuggingFaceToolCallingLLM.query`：渲染消息、生成、解析调用和写入 activation；
   - `_ResidualPreCapture`：在 decoder block 上注册 `forward_pre_hook`；
   - `tool_input`：首次 generation prefill 的最后非 padding token；
   - `function_call`：沿原始 generated token IDs replay 到调用 closing tag。

4. [`agentdojo_integration.py`](../src/agent_defense/agentdojo_integration.py)

   - `GuardedToolsExecutor.query`：本项目唯一的 pre-action enforcement；
   - block 分支只构造 `status=blocked` 的 tool result；
   - allow 分支才会调用 `runtime.run_function`。

5. [`policy.py`](../src/agent_defense/policy.py)

   - `ToolRiskPolicy.classify`：名称级、确定性的工具影响分级；
   - `RuntimeGate.decide`：把 detector observation 与工具风险合成最终决策。

6. [`detectors.py`](../src/agent_defense/detectors.py)

   - `DirectionDetector`：difference-in-means direction 的 cosine/projection score；
   - `LinearProbeDetector`：StandardScaler + logistic probe；
   - `MelonToolCallDetector`：比较原调用与 masked re-execution 的候选调用。

最后才看 [`matrix.py`](../src/agent_defense/matrix.py) 的 `apply_call_reviews` 和
`aggregate_results`；它们解释正式实验怎样从 episode 结果得到 BU、UA、ASR 和人工审核指标，
不是 runtime 安全边界本身。
...
Artifact 绑定模型、revision、dtype、layer、position、chat template 和 tool schema 等身份。不兼容时 observation
invalid；高风险动作按 fail-closed 处理，不能静默复用。
...
Masked generation 发生在 scoring 前，所以它的主要开销不在 `detector_latency_ms` 中，但会进入 episode
端到端时间和模型调用数。

### No defense

`NoDefenseDetector` 始终返回有效且不触发的 observation；自定义 executor 仍保留完整 trace。因此 none
可以与 direction/probe/MELON 做同粒度调用审计。

AgentDojo built-in `repeat_user_prompt` 使用上游 executor，当前没有本项目的 per-call trace，所以调用级
interception/false-block 必须是 `N/A`。

## 6. 最容易讲错的术语

| 术语 | 正确含义 |
|---|---|
| `tool_input` | CLI 名称；artifact 中精确记为 `generation_prefill_last_nonpad` |
| `function_call` | CLI 名称；artifact 中精确记为 `function_call_end` |
| `triggered` | detector 分数超过阈值，不保证最终 block |
| detector `valid` | detector 是否健康，不表示 episode 安全 |
| trial `valid` | 基础设施、解析和 detector 健康，不表示 utility 通过 |
| `runtime_invoked` | 调用已经进入真实 runtime，可能产生副作用 |
| `tool_succeeded` | runtime 返回时是否无错误 |
| `executed` | `runtime_invoked` 的兼容别名，不等于成功 |
| raw `security_results=True` | injection task 的攻击目标已完成，即一次 ASR 命中 |
| `activation_probe` | defense 展示名；artifact/detector kind 是 `linear_probe` |

## 7. 三种演示不要混用

| 入口 | 用途 | 能否进真实效果表 |
|---|---|---|
| [`demo.py`](../src/agent_defense/demo.py) | 脚本化 Agent + synthetic activation，教学控制流 | 否 |
| [`agentdojo_runner.py`](../src/agent_defense/agentdojo_runner.py) | 真实 AgentDojo runtime contract，证明 block 早于副作用 | 否 |
| [`experiments.py`](../src/agent_defense/experiments.py) + [`matrix.py`](../src/agent_defense/matrix.py) | 真实 HF episode 与 frozen held-out 聚合 | 是 |

## 8. 建议断点

按一次 attacked case 设置这些符号断点即可：
...
4. `LinearProbeDetector.inspect` 或 `MelonToolCallDetector.inspect`：看 score/threshold；
5. `RuntimeGate.decide`：看 risk 与 observation 如何合成；
...
- train 拟合 scaler/weights；
- benign calibration 选 threshold；
- test 只在设计冻结后运行一次；
- 同一任务的 clean/attack pair 不能跨 split。
...
tool-call boundary、hook token、call-level 标签、数据泄漏、MELON 与 probe 的权衡。
