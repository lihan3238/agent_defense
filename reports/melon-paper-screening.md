# MELON（Masked re-Execution and TooL comparisON，掩码重执行与工具调用比较）论文兼容路径：Qwen3-8B（通义千问 3 80 亿参数模型）筛选报告

> **后续状态：**这是完整主矩阵之前的历史筛选记录。后续已使用独立冻结的完整清单运行四套件 629 个攻击配对，
> 正式效果只看 [Qwen3-8B 四套件主矩阵](qwen3-v112-full-matrix.md)；本报告继续保存筛选阶段的诊断和停止依据。
...
下表的 BU（Benign Utility，良性任务可用性）、UA（Utility Under Attack，攻击场景任务可用性）和
Targeted ASR（Targeted Attack Success Rate，定向攻击成功率）只使用有效回合做分母：
...
2. 在该 16 配对筛选中，Qwen3-8B + MiniLM 配置没有给出可靠效果证据：掩码轨迹很少生成候选，唯一阻断又不是精确攻击参考调用。

因此筛选阶段当时停止在 16 配对，没有把同一批已观察案例直接扩写成效果结论。后续工作先增加 Matrix Schema v2
（矩阵清单模式第二版）、四套件 runner（运行器）、受检合并与分析脚本，再按新冻结协议运行完整分母；它没有回填、替换或
重报本报告的 64 个筛选回合。完整主矩阵仍使用本地 MiniLM 和单一攻击，因此同样不能称为论文数值复现。
