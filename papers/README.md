...
PDF（Portable Document Format，便携式文档格式）SHA-256（Secure Hash Algorithm 256-bit，256 位安全哈希算法）
校验值见 [`SHA256SUMS`](SHA256SUMS)，可在 `papers/` 下运行
...
2. **再读 MELON（Masked re-Execution and TooL comparisON，掩码重执行与工具调用比较）§3、
   Algorithm 1（算法 1）。**它是本项目的顶会复现线，负责
   masked re-execution（掩码重执行）与动作提交前的 tool-call comparison（工具调用比较）。

AgentDojo 已经看过时，把它当 benchmark（基准测试）/API（Application Programming Interface，应用程序编程接口）手册按需查；
...
- 出处：NeurIPS（Conference on Neural Information Processing Systems，神经信息处理系统大会）2024
  Datasets and Benchmarks（数据集与基准测试）；
...
- 出处：ICML（International Conference on Machine Learning，国际机器学习大会）2025；
...
- 决定：仓库同时保留 `melon` 哈希切片与 `melon_paper` 论文兼容重建；后者已完成四套件单攻击完整主矩阵，
  但不冒充论文嵌入后端、四攻击或原始数值复现。
...
- 出处：[arXiv v1](https://arxiv.org/abs/2607.12624)；作者稿标注 ACM MM（Association for Computing Machinery
  International Conference on Multimedia，美国计算机协会国际多媒体会议）2026，但当前 PDF 的
  DOI（Digital Object Identifier，数字对象标识符）
  仍为占位符；[官方代码](https://github.com/Claresigle/PVDetector)
...
- 限制：论文面向 purpose-specific agent（特定用途智能体）的输入级 PI（Prompt Injection，提示注入）检测，不等同于
  AgentDojo 的 indirect PI（间接提示注入）多步轨迹；迁移是否成立必须实验验证。

## P1（Priority 1，第一优先级）：读方法与撞车边界
...
- 出处：ACL（Annual Meeting of the Association for Computational Linguistics，计算语言学协会年会）2025
  Long Paper（长文）；[arXiv](https://arxiv.org/abs/2412.16682)
- 优先阅读：§3、§4、§5。
...
2. **固定执行边界：**检测信号可以来自重执行或隐藏状态，但真正的 allow/block（放行/阻断）必须发生在候选工具调用
   产生后、工具尚未执行时。
3. **当前实现：**同一 AgentDojo 工程中已有 `none`、`repeat_user_prompt`、`direction`、
   `activation_probe`、MELON slice（MELON 算法切片）和 `melon_paper`。30 回合表示级 held-out（留出测试）矩阵
   直接对应简历中的隐藏状态探针；四套件完整主矩阵记录论文兼容路径的单攻击效果与代价。
...
5. **贡献表述：**这是受前沿工作启发的独立工程化迁移与可复现实验框架，不声称发明 hidden-state Agent 防御，
   也不把单攻击 8B 结果称为跨场景验证。当前可讨论的增量是统一动作边界、可审计适配、完整分母下的本地重跑和
   诚实的失败分析；正式记录入口见 [`reports/README.md`](../reports/README.md)。
